...
class Employee:
    ...
    def Calculate_Salary(self, emp_id):
        ...
        if result is not None:
            ...
            hra = 0.2 * (base_salary or 0)
            da = 0.1 * (base_salary or 0)
            gross_salary = ((base_salary + hra + da) or 0)
            tax = 0.1 * (gross_salary or 0)
            net_salary = ((gross_salary - tax) or 0)
            return {
...
class Admin:
    ...
    def delete_employee(self, del_id):
        self.cursor.execute("DELETE FROM attendance WHERE Employee_ID = %s", (del_id,))
        self.cursor.execute("delete from Employee_Data where ID = (%s)", (del_id,))
        self.db.commit()

    def update_employee(self, upd_id, name, designation, intage, salary, address, joining_date):
        self.cursor.execute("""
                            
    UPDATE Employee_Data 
    SET (Name=%s, Designation=%s, Age=%s, Salary=%s, Address=%s, Joining_date=%s)
    WHERE ID=%s
""", (name, designation, intage, salary, address, joining_date, upd_id))
        self.db.commit()
...
def page_admin_panel():
    ...
    if menu == "Select":
        ...
        if submitted:
            ...
            st.session_state.page = None
            st.success("Logout Successfully! Thanks for using Employee Payroll System.")
...
def page_employee_panel():
    ...
    if choice == "View Personal Details":
        ...
        st.session_state.page = None
        st.write("Logout Successfully! Thanks for using Employee Payroll System ")
